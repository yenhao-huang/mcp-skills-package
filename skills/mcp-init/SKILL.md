---
name: mcp-init
description: Initialize or repair OpenAI Codex MCP configuration for Jina MCP, Firecrawl MCP, and Git MCP. Use when the user asks to set up MCP servers, add Jina/Firecrawl/Git MCP to Codex, bootstrap ~/.codex/config.toml, refresh MCP server blocks, or create a repeatable local MCP initialization workflow.
---
...
Use this skill to configure `~/.codex/config.toml` with three MCP servers:
...
4. Ask for API keys only if the user wants you to validate connectivity. For configuration only, use `JINA_API_KEY` and `FIRECRAWL_API_KEY` variable names without seeing the secret values.
